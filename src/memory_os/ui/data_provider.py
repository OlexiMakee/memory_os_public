...
class DefaultGraphDataProvider(IGraphDataProvider):
    ...
    def _resolve_file_path(self, file_path_str: str, project_root: Path) -> Optional[Path]:
        ...
        if not str(target_path).startswith(str(project_root)):
            return None  # Security violation
        ...
        found = [f for f in found if not any(part.startswith('.') or part == '__pycache__' or part == 'venv_auto' for part in f.parts)]
    ...
    def read_evidence_file(self, file_path: str) -> Optional[bytes]:
        ...
        Returns bytes if successful, None if the file is outside boundaries or unreadable.
        """
        try:
            project_root = self.config.root_dir.resolve()
