...
Метод [compact_capsules](src/memory_os/modules/compactor.py#L115) класу [MemoryCompactor](src/memory_os/modules/compactor.py#L86) реалізує покрокове вилучення знань:
...
LLM за допомогою [SYSTEM_PROMPT](src/memory_os/modules/compactor.py#L17) повертає JSON із пропозиціями нових вузлів та зв'язків.
...
Коли в системі накопичується багато верифікованих вузлів, деякі правила можуть дублювати або перекривати одне одного. Метод [compress_graph](src/memory_os/modules/compactor.py#L374) вирішує цю проблему:

1. Збирає всі верифіковані вузли знань (`status == 'verified'`). Якщо їх менше двух, стиснення не запускається.
2. Передає їхній список до LLM із промптом [COMPRESSION_PROMPT](src/memory_os/modules/compactor.py#L58).
...
Для економії токенів розробника під час роботи агента, великі файли капсул мають очищуватися. Метод [archive_compacted_capsules](src/memory_os/modules/compactor.py#L485) працює так:
