...
Граф знань Memory OS є динамічним. Щоб запобігти накопиченню помилок та застарілих правил, у системі реалізовано скінченний автомат керування станами пам'яті через клас [LifecycleManager](src/memory_os/modules/lifecycle.py#L14).
...
* Метод [_validate_evidence](src/memory_os/modules/lifecycle.py#L38) проходить по кожному рядку в масиві `evidence`.
...
Для запобігання безконтрольному росту графу та економії пам'яті LLM, застарілі дані періодично архівуються через метод [prune()](src/memory_os/modules/lifecycle.py#L224).
